...
TASK=TASK.replace('char_tfidf, construction_token_tfidf, and word_tfidf at k=50', 'char_tfidf, construction_token_tfidf, word_tfidf, bm25_token, and char_word_fusion at k=5')
TASK=TASK.replace('selection="fused" and candidate_limit=50', 'selection="fused" and candidate_limit=10')
...
TASK=TASK.replace('candidate_limit=30', 'candidate_limit=10')
...
TASK += ' The manual retrieval setup is authoritative: use exactly the five retrievers char_tfidf, construction_token_tfidf, word_tfidf, bm25_token, and char_word_fusion with k=5 and fusion=rrf; review the fused top-10 shortlist with selection=fused and candidate_limit=10.'
...
TASK += ' Request evidence for each review ID exactly once. Never retry get_candidate_evidence for an ID, never alter or truncate artifact_id, and if an evidence call fails, stop the assigned unit rather than issuing a second evidence call.'
...
def review(config, task=''):
    ...
    checks={
      ...
      'bounded_fused_review':'candidate_limit=10' in text and 'selection=fused' in text,
      ...
      'no_heuristic_gate':('deterministic gate' in text or 'deterministic disagreement gate' in text) and 'do not use' in text and 'margin' in text,
