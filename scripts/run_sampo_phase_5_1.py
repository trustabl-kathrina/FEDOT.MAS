...
TASK=TASK.replace('selection="fused" and candidate_limit=50; this exposes a broader bounded shortlist without bulky per-method provenance. Request evidence for no more than two examples at a time', 'selection="diverse_round_robin" and candidate_limit=20; this preserves artifact-local candidate indices while giving the reviewer a diverse bounded shortlist. Request evidence for no more than six examples at a time')
...
def review(config):
    ...
    checks={
      ...
      'bounded_fused_review':("selection='fused'" in text or 'selection=\\"fused\\"' in text) and 'candidate_limit=50' in text,
