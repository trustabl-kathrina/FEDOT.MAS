...
def malformed_tool_call(raw, allowed_ids):
 ...
 for call in raw.get('tool_calls',[]):
  ...
  if call.get('tool')=='save_review_decisions' and os.environ.get('PHASE5_EXPERIMENTAL_TAIL')=='1' and not call.get('fill_retrieval_tail'): return True
  if call.get('tool')=='get_candidate_evidence' and (len(call.get('ids') or [])>4 or (call.get('evidence') or {}).get('selection')!='fused' or (call.get('evidence') or {}).get('candidate_limit') != 10): return True
 return False
def main():
