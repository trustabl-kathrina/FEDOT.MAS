...
def main():
 ...
 OUT.mkdir(parents=True); meta=json.loads((CONFIG/'metadata.json').read_text()); run=meta['run_id']; prediction=ROOT/f'artifacts/sampo_benchmark/mas_runs/{run}.jsonl'
 ...
 for offset in (0,20,40):
  ...
  env=dict(os.environ,PHASE5_CONFIG=CONFIG.name.removeprefix('config_'),PHASE5_ASSIGNED_IDS=json.dumps(assigned),PHASE5_OFFSET=str(offset),PHASE5_ENFORCE_COMPLETE='1',PHASE5_COMPLETION_SENTINEL=str(sentinel),PHASE5_TRACE_PATH=str(trace),PHASE5_RESULT_PATH=str(result))
  ...
  item={'offset':offset,'assigned_ids':assigned,'runtime_seconds':time.monotonic()-start,'stored_ids':sorted(final & set(assigned)),'outside_ids':sorted(outside),'max_prompt_tokens':max([x['prompt_tokens'] for x in raw['model_calls']],default=0),'model_calls':len(raw['model_calls']),'tool_calls':len(raw['tool_calls']),'duplicate_persistence':duplicate_persistence(raw),'repeated_prepare':repeated_prepare(raw),'malformed_tool_call':malformed_tool_call(raw),'complete':set(assigned)<=final,'sentinel':sentinel.exists(),'failures':raw['failures']}
