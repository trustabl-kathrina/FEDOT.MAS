...
def malformed_tool_call(raw, allowed_ids):
 ...
 for call in raw.get('tool_calls',[]):
  ...
  if call.get('tool')=='get_candidate_evidence' and (len(call.get('ids') or [])>4 or (call.get('evidence') or {}).get('selection')!='diverse_round_robin' or not 1 <= (call.get('evidence') or {}).get('candidate_limit',0) <= 30): return True
 return False
def main():
 ...
 for offset in (0,20,40):
  ...
  env=dict(os.environ,PHASE5_CONFIG=CONFIG.name.removeprefix('config_'),PHASE5_RUN_ID=run,PHASE5_ASSIGNED_IDS=json.dumps(assigned),PHASE5_OFFSET=str(offset),PHASE5_ENFORCE_COMPLETE='1',PHASE5_COMPLETION_SENTINEL=str(sentinel),PHASE5_TRACE_PATH=str(trace),PHASE5_RESULT_PATH=str(result))
  ...
  item={'offset':offset,'assigned_ids':assigned,'runtime_seconds':time.monotonic()-start,'stored_ids':sorted(final & set(assigned)),'outside_ids':sorted(outside),'max_prompt_tokens':max([x['prompt_tokens'] for x in raw['model_calls']],default=0),'model_calls':len(raw['model_calls']),'tool_calls':len(raw['tool_calls']),'duplicate_persistence':duplicate_persistence(raw,set(assigned)),'repeated_prepare':repeated_prepare(raw),'malformed_tool_call':malformed_tool_call(raw,set(assigned)),'complete':set(assigned)<=final,'sentinel':sentinel.exists(),'failures':raw['failures']}
  report['batches'].append(item)
  if not item['complete'] or item['outside_ids'] or item['duplicate_persistence'] or item['repeated_prepare'] or item['malformed_tool_call'] or item['max_prompt_tokens']>64000 or item['model_calls']>30 or item['tool_calls']>60 or item['failures']:
